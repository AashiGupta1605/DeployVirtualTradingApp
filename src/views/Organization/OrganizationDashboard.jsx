...
export default function OrganizationDashboard({ type }) {
  ...
  useEffect(() => {
    ...
    const fetchTotalUsers = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/org/user/count/total");
        setTotalUsers(response.data.count);
      } catch (error) {
        console.error("Error fetching total user count:", error);
      }
    ...
    const fetchNewUsersLastWeek = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/org/user/count/new-week");
        setNewUsersLastWeek(response.data.count);
      } catch (error) {
        console.error("Error fetching new users count from last week:", error);
      }
