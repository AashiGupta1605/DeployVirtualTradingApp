...
export default function ETFTable() {
  ...
  const cardConfigurations = [
    {
      subtitle: "Virtual Amount",
      title: activeSubscription.virtualAmount
        ? `${activeSubscription.virtualAmount.toFixed(2)}₹`
        : "0.00₹",
      iconName: "fas fa-wallet",
      iconColor: "bg-indigo-500",
      percentColor: "text-gray-500",
    ...
    {
      subtitle: "Trading Performance",
      title: statistics.successRate
        ? `${statistics.successRate.toFixed(2)}%`
        : "0.00%",
      iconName: "fas fa-chart-pie",
      iconColor: "bg-orange-500",
      percentColor:
        (statistics.successRate || 0) >= 50 ? "text-emerald-500" : "text-red-500",
      description: "Success Rate",
    },
    {
      subtitle: "Trades & P&L",
      title: statistics.buyTrades && statistics.sellTrades
        ? `${statistics.buyTrades + statistics.sellTrades}`
        : "0",
      iconName: "fas fa-exchange-alt",
      iconColor: "bg-pink-500",
      percentColor:
        (statistics.realizedPLPercentage || 0) >= 0
          ? "text-emerald-500"
          : "text-red-500",
      description: `Realized P&L: ${
        statistics.realizedPL ? statistics.realizedPL.toLocaleString() : "0.00"
      }%`,
    },
  ];
      ...
      <div className="mt-24">
        <div className="bg-lightBlue-600 md:pt-8 pb-16 pt-12">
          <div className="px-4 mx-auto w-full">
            <div className="flex flex-wrap">
              {cardConfigurations.map((card, index) => (
                <div key={index} className="w-full lg:w-6/12 xl:w-3/12 px-4 mb-4">
                  <CardStats
                    statSubtitle={card.subtitle}
                    statTitle={card.title}
                    statArrow={parseFloat(card.title) >= 0 ? "up" : "down"}
                    statPercent={
                      card.subtitle === "Trading Performance"
                        ? card.title
                        : Math.abs(statistics.realizedPLPercentage || 0).toFixed(2)
                    }
                    statPercentColor={card.percentColor}
                    statDescription={card.description}
                    statIconName={card.iconName}
                    statIconColor={card.iconColor}
                  />
                </div>
