...
const StockDetailsModal = ({ 
  ...
  // State for CompanyDetailModal
  const [showCompanyModal, setShowCompanyModal] = useState(false);

  // Memoized calculations for stock statistics
  const stockStats = useMemo(() => {
    ...
    const realizedPL = sellTransactions.reduce((total, transaction) => {
      const correspondingBuy = buyTransactions.find(
        buy => buy.companySymbol === transaction.companySymbol
      );
      
      if (correspondingBuy) {
        const profitLoss = (transaction.price - correspondingBuy.price) * transaction.numberOfShares;
    ...
    // Calculate unrealized Profit/Loss (if current market price is available)
    const unrealizedPL = stock.holding 
      ? (stock.currentMarketPrice - stock.holding.averageBuyPrice) * stock.holding.quantity 
      : 0;

    return {
      totalShares: stock.holding?.quantity || 0,
      averagePrice: stock.holding?.averageBuyPrice || 0,
      totalInvestment: (stock.holding?.quantity || 0) * (stock.holding?.averageBuyPrice || 0),
  ...
  // Handle Company Details Modal Opening
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 p-4 rounded-lg border border-indigo-100">
              <h4 className="text-sm text-indigo-600 mb-2">Trade Summary</h4>
            ...
            <div className="bg-gradient-to-br from-red-50 to-red-100 p-4 rounded-lg border border-red-100">
              <h4 className="text-sm text-red-600 mb-2">Performance</h4>
              <div className="flex items-center">
                <span className={`text-xl font-bold ${
                  stockStats.realizedPL >= 0 ? 'text-green-600' : 'text-red-600'
                }`}>
                  ₹{stockStats.realizedPL.toFixed(2)}
                </span>
                <span className={`ml-2 text-sm ${
                  stockStats.realizedPL >= 0 ? 'text-green-500' : 'text-red-500'
                }`}>
                  Realized P&L
                </span>
              </div>
