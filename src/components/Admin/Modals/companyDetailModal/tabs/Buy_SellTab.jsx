...
const Buy_SellTab = ({ symbol, data, loading, error }) => {
  ...
  const activeSubscription = useMemo(() => {
    ...
    console.log('Active Subscription Selection:', {
      userSubscriptions,
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const subscriptionsResult = await dispatch(getUserSubscriptions(userId));
        
        console.log('Subscriptions Fetch Result:', {
          userId,
          subscriptionsResult: subscriptionsResult.payload
        });

        // Find active subscription
        const activeSubscriptionId = subscriptionsResult.payload?.data?.find(sub => 
        ...
        if (activeSubscriptionId) {
          // Fetch holdings
          const holdingsResult = await dispatch(fetchHoldings({
            userId, 
            subscriptionPlanId: activeSubscriptionId
          }));
          
          // Fetch transaction history
          const historyResult = await dispatch(fetchTransactionHistory({ 
            userId, 
            subscriptionPlanId: activeSubscriptionId 
          }));

          console.log('Holdings Fetch Result:', holdingsResult);
          console.log('Transaction History Result:', historyResult);
        } else {
          console.warn('No active subscription found');
          setLocalError('No active subscription found');
        }
      } catch (err) {
        console.error('Error fetching data:', err);
        setLocalError(err.message || 'Failed to load trading data');
  ...
  // Price and Market Price Effects
  useEffect(() => {
  ...
  const handleQuantityChange = useCallback((value) => {
  ...
  }, [activeTab, calculateRemainingBalance, currentMarketPrice, currentHolding]);
    ...
    if (localLoading || loading || tradingLoading) {
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1 bg-gray-50 rounded-xl p-6" style={{ flex: '0 0 60%' }}>
            ...
            <TradingControls
              ...
              isDisabled={loading || tradingLoading}
