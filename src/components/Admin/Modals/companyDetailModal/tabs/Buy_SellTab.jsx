...
const Buy_SellTab = ({ symbol, data, loading, error }) => {
  ...
  // Add state for SubscriptionModal
  ...
  useEffect(() => {
    // Only dispatch if userId exists
    if (userId) {
      dispatch(getUserSubscriptions(userId));
      dispatch(fetchHoldings(userId));
      
      // Additional null check for activeSubscription
  ...
  const handleViewSubscriptionPlans = () => {
    console.log('View Subscription Plans clicked');
    console.log('Current userData:', userData);
    
    // Add a toast to verify the click is working
    toast.success('Opening Subscription Modal', {
      position: 'top-right'
    });
