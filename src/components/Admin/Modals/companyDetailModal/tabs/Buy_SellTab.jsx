...
//                 {/* add dynammic balalnce and no. of shares */}
//               </div>
//               <div className="bg-gray-50 rounded-xl p-4">
...
//  const userId = user._id;
 const dispatch = useDispatch();
  ...
  const handlePlaceOrder = (orderDetails) => {
    if (orderDetails.type === 'buy' && orderDetails.total > balance) {
      alert('Insufficient balance');
      return;
    }

    if (orderDetails.type === 'sell' && orderDetails.quantity > holdings) {
      alert('Insufficient holdings');
      return;
    }

    if (orderDetails.type === 'buy') {
      dispatch(buyStock(orderDetails));
    } else {
      dispatch(sellStock(orderDetails));
    }
  ...
  return (
    <div className="w-full bg-white rounded-xl shadow-lg p-6 space-y-6">
      ...
      <TradingControls
        ...
        currentPrice={currentPrice}
        onPlaceOrder={handlePlaceOrder}
        maxQuantity={activeTab === 'sell' ? holdings : Math.floor(balance / currentPrice)}
