...
// Replace the existing renderChartTab function with this one
...
// In CompanyDetailModal.jsx, update the fetchHistoricalData function:
...
const fetchHistoricalData = async (timeRange = '') => {
  try {
    ...
    let endpoint;
    if (activeTab === 'chart') {
      // Use chart endpoint for chart data
      endpoint = type === 'etf'
        ? `${BASE_API_URL}/admin/etf/chart/${symbol}`
        : `${BASE_API_URL}/admin/nifty/company/chart/${symbol}`;
    } else {
      // Use history endpoint for table data
      endpoint = type === 'etf'
        ? `${BASE_API_URL}/admin/etf/history/${symbol}`
