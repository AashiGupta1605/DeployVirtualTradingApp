...
import { FaRegSadTear, FaSpinner, FaBuilding } from "react-icons/fa";
            ...
            <div className="mb-4 flex justify-between items-center">
                <div className="bg-gray-100 px-3 py-1 rounded-lg">
                    <span className="text-xs font-medium text-gray-700">
                        Partners: <span className="font-bold text-lightBlue-600">{clientsData.length}</span>
                    </span>
                </div>
                <button 
                    onClick={openModal}
            ...
            <div className="grid grid-cols-2 gap-3">
                {clientsData.slice(0, 6).map((client, index) => (
                    <motion.div 
                        ...
                        className="bg-white rounded-lg p-2 h-28 flex items-center justify-center border border-gray-100 hover:border-lightBlue-200 transition-all"
                    >
                        <img
                            src={client.photo || "https://via.placeholder.com/150?text=Client+Logo"}
                            alt={client.name}
                            className="h-16 w-16 rounded-full object-cover filter grayscale hover:grayscale-0 opacity-80 hover:opacity-100 transition-all"
                            onError={(e) => {
                                e.target.src = "https://via.placeholder.com/150?text=Client+Logo";
                                e.target.className = "h-16 w-16 rounded-full object-cover opacity-70";
                            }}
                        />
                    </motion.div>
                ))}
        ...
        <div className="relative px-4">
            ...
            <Slider {...settings} className="pb-6">
                {clientsData.map((client, index) => (
                    ...
                    >
                        <div className="bg-white rounded-lg p-3 h-36 flex items-center justify-center border border-gray-100 hover:border-lightBlue-200 transition-all">
                            <img
                                src={client.photo || "https://via.placeholder.com/150?text=Client+Logo"}
                                alt={client.name}
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <AnimatePresence mode="wait">
                    {loading ? (
                        ...
                        <>
                            {isMobile ? (
                                ...
                                <div className="relative px-4 lg:px-8">
                                    ...
                                    <Slider {...settings} className="pb-6 lg:pb-8">
                                        {clientsData.map((client, index) => (
                                            <motion.div 
                                                key={index}
                                                whileHover={{ scale: 1.05 }}
                                            ...
                                            >
                                                <div className="bg-white rounded-lg p-4 h-40 flex items-center justify-center border border-gray-100 hover:border-lightBlue-200 transition-all">
                                                    <img
                                                        ...
                                                        onError={(e) => {
                                                            e.target.src = "https://via.placeholder.com/150?text=Client+Logo";
                                                            e.target.className = "h-32 w-32 rounded-full object-cover opacity-70";
                                                        }}
                                                    />
                                                </div>
                                            </motion.div>
                        ...
                        >
                            <FaBuilding className="text-3xl text-gray-400 mb-4" />
